...
The goal of stats is to extract, from the dataset, estimates of: parameters, features (statistics), or even all, of the "true" distribution. As example, for a Poisson distribution, getting the mean of the dataset estimates $\lambda$, the average rate of events per unit time/space (since $E[X]=\lambda$). For exponential, $E[X]=\frac{1}{\lambda}$ (estimate $\lambda$). For binomial, $E[X]=np$ (estimate $n$ or $p$). Etc. Sample points of curves in excel and compare to a histogram of the dataset.

(clarify) The law of large numbers says that the more data points you have, the closer the sample/empirical statistic is to the "true" distribution's feature. The central limit theorem says how much it varies with $n$.

**Statistical model 2** (of bivariate data), called "regression model": The dataset $(x_1,y_1)$, ..., $(x_n,y_n)$ is modeled as a realization of randvars $Y_1$, ..., $Y_n$ such that: $Y_i=g(x_i) + U_i$ and the $U_i$s are independent with mean 0 and same finite variance (random fluctuation). The $Y_i$s do NOT form a random sample, as their means vary, hence they're not iid.
