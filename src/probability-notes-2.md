...
: *Discrete*: $p_{X,Y}(x,y)=P(X=x,Y=y)$. It reads as: "the probability that $X$ takes on the value $x$ *and* $Y$ takes on the value $y$". \
  *Continuous*: $P((X,Y)∈A) = \iint_A f(x,y)\,dx\,dy$ where $f$ is a function from $\R^2 \rightarrow \R$ such that probability remains positive for all $A \subseteq \R^2$, and adding up to 1 over the entire sample space.
...
: $p_X(x)=P(X=x,Y≤\infty)$ \
  $p_Y(y)=P(X≤\infty,Y=y)$ \
  (For continuous there's marginal density functions.) Both cases have marginal distribution functions. Marginal functions are so called because they are found in the "margins" of a joint distribution table.
...
**Sum of normal**: $X\sim\sum_i N(\mu_i,\sigma_i^2)=N(\sum_i\mu_i,\sum_i\sigma_i^2+...)$
