...
product of the probabilities (ie $f_{X,Y}(x,y)=f_X(x)f_Y(y)$). For the sum $Z=X+Y$, you have that $F_Z(z)=P(Z\leq z)=P(X+Y\leq z)=\iint_{x+y\leq z} f_{X,Y}(x,y)\,dx\,dy$. Then, for the sum of two independent variables:
...
(The last line is obtained from the previous one with something called Leibniz's rule for differentiation under the integral?)
