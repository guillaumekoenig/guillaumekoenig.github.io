...
Scale, in ascending order: atom < base (ACGT in DNA, ACGU in RNA) < nucleotide < nucleic acid < codon (3 RNA nucleotides). Hence there are 64 possible codons, that encode 20 amino acids (molecular compounds).
...
mRNA is the kind that DNA is copied into and worked on by the ribosome (m for messenger). The codons need to be translated into amino acids: that's done with tRNA, molecules having the *anticodon* that matches the codon (by base-pairing) and holding the associated amino acid. mRNA is short-lived (minutes). Ribosomal RNA (discussed later) is much more prevalent, and long-lived.

It was experimentally verified that codons come in 3 nucleotides the following way. Insert a single nucleotide, and the resulting protein is deadly. Same if you insert 2 nucleotides. However insert 3, and you get a mostly functional protein.
