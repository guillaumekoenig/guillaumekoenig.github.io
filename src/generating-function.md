...
Table: *Some classic recurrence relations*
...
| Combinations | $p(n-1,k-1) + p(n-1,k)$ | Either use element from set of $n$ this one time, or skip it |
...
- Kadane: maximum subarray sum. Consider the max of all suffix subarrays ending at $j$: $p(j)=max(p(j-1)+arr[j],arr[j])$. Then you're looking for the max over all $j$s.
