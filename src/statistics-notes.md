...
**Sample statistic**: $h(X_1,...,X_n)$ ($h$ is a "summary function"), more precisely it refers to the value once a sample is taken (so it's not a random variable).
...
An **estimate** $t$ is a value, function of a dataset (which is not necessarily the realization of a random sample): \[t=h(x_1,...,x_n)\]. Note estimate and sample statistic refer to the same thing.
