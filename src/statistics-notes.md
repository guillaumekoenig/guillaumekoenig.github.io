...
In practice you take the derivative of $h$ and find its zeroes to find the maximum. Or you inspect $h$ if it is not smooth (for example estimating the endpoint of uniform involves taking the max, which leaves you with a discontinuity). In one exercise you estimate the maximum likehood of the probability of heads of an unfair coin, given throw experiments.
...
That maximum is going to be a function of $(x_1, ..., x_n)$, say $h$. The corresponding randvar $T=h(X_1, ..., X_n)$ is the **maximum likelihood estimator** for $\theta$.
