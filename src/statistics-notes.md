...
- **Sample mean**: $\bar{x}_n=\frac{x_1+x_2+...+x_n}{n}$
- **Sample standard deviation**: $s_n=\sqrt{\frac{1}{n-1}\sum_i (x_i-\bar{x}_n)^2}$ (Why $n-1$?)
...
The empirical quantile function is defined on $[0,1]$, the percentile function on $[0,100]$ (more common). More precisely, the domain is split evenly with the number of data points, such that $q(\frac{i}{n+1})=x_{(i)}$, where the $x_{(i)}$s are the so called order statistics (the data points reordered such that they are sorted). The range of the quantile and percentile functions is the interval between the min and max values of the data points. The p0 percentile maps to the min value ($x_{(1)}$), p100 to the max value ($x_{(n)}$). The domain of $\left(\frac{1}{n+1},\frac{n}{n+1}\right)$ not falling on exact proportions is mapped by linear interpolation of the two closest quantiles. Experiment with numpy.
...
The goal of stats is to extract, from the dataset, estimates of: parameters, features (statistics), or even all, of the "true" distribution. As example, for a Poisson distribution, getting the mean of the dataset gives an estimate of $\lambda$, the average rate of events per unit time/space (since $\E{X}=\lambda$). For exponential, $\E{X}=\frac{1}{\lambda}$ (estimate $\lambda$). For binomial, $\E{X}=np$ (estimate $n$ or $p$). For geometric, $\E{X}=\frac{1}{p}$. Etc. Sample points of curves in excel and compare to a histogram of the dataset.
...
The dataset $x_1$, ..., $x_n$ is a realization of the random sample $X_1$, ... $X_n$ drawn from $F$, but $F$ is unknown. Instead the dataset can be used to build an estimate $\hat{F}$ distribution. From $\hat{F}$ you build a "bootstrap" random sample $X_1^*$, ..., $X_n^*$, and the "bootstrapped" sample statistic $h(X_1^*, ..., X_n^*)$, whose distribution serves to estimate that of $h(X_1, ..., X_n)$ (since $F$ is unknown).

## Empirical bootstrap (ie non-parametric)

In the empirical bootstrap, $\hat{F}$ is simply the empirical distribution function $F_n$, where each element $x_1$, ..., $x_n$ occurs with probability $1/n$.

Draw 1000 realizations of $X_1^*$, ..., $X_n^*$ from $\hat{F}$. For each, compute $h(x_1^*, ..., x_n^*)$: their distribution serves to estimate that of $h(X_1, ..., X_n)$.

There's a math argument that estimations work better with *centered* statistics, eg $\bar{X}_n-\mu$ and also $\Med(X_1, ..., X_n)-F^{\text{inv}}(0.5)$. (Yes $F^{\text{inv}}(0.5)$ is the median of the true distribution.)

Note with the 1000 realizations you can also compute an estimate for: $P(|\bar{X}_n - \mu| > 5)$, ie the probability that the average of the sample from the true mean is greater than 5. (Only in likeliness... $F$ is still unknown.)

## Parametric bootstrap

In the parametric bootstrap, $\hat{F}$ is obtained by estimating the parameter(s) $\theta$ of the true distribution, ie $\hat{F}=F_{\hat{\theta}}$. Then you again draw 1000 realizations of $X_1^*$, ..., $X_n^*$ from $F_{\hat{\theta}}$ to build a distribution estimate for the (centered) statistic of interest.

Here's an intriguing statistic. In this situation you have both an empirical distribution function $F_n$ and a parametric one $F_{\hat{\theta}}$. You can compute the max distance between them. This is the Kolmogorov-Smirnov distance.
...
Is $V_n^2=\frac{1}{n}\sum_i (X_i-\bar{X}_n)^2$ an unbiased estimator of the underlying distribution's $\sigma^2$?

\begin{align*}
\E{V_n^2} &= \frac{1}{n}\sum_i \E{(X_i-\bar{X}_n)^2} \\
&= \frac{1}{n}\sum_i \Var{X_i-\bar{X}_n} + (\E{X_i-\bar{X}_n})^2 \\
&= \frac{1}{n}\sum_i \Var{X_i-\bar{X}_n} \\
&= \frac{1}{n}\sum_i \Var{\frac{n-1}{n}X_i-\sum_{j\neq i}\frac{X_j}{n}} \\
&= ... \\
&= \frac{n-1}{n}\sigma^2
\end{align*}

The answer is no: $\E{V_n^2}<\sigma^2$ and hence $V_n^2$ is a negatively biased estimator of $\sigma^2$. However, $S_n^2=\frac{1}{n-1}\sum_i (X_i-\bar{X}_n)^2$ is an unbiased estimator of $\sigma^2$, since $\E{S_n^2}=\sigma^2$ (by a similar calculation as above). That's the reason of the $n-1$ denominator for the sample variance.
...
In other words, what value $\hat{\theta}$ maximizes $L(\theta)=P(X_1=x_1,...,X_n=x_n)$, ie maximizes (assuming random sample):

*Discrete*: $p_\theta(x_1)p_\theta(x_2)...p_\theta(x_n)$\
*Continuous* (*): $f_\theta(x_1)f_\theta(x_2)...f_\theta(x_n)$

(*) Justification for continuous: $\prod P(x_i-\varepsilon\leq X_i\leq x_i+\varepsilon) \approx \prod 2\varepsilon f_\theta(x_i)$ and you can drop the $2\varepsilon$ since it doesn't impact finding the max.
...
With some linear algebra this gives you a (somewhat unwieldy) expression for $\hat{\beta}$. And then $\hat{\alpha}=\bar{y}_n-\hat{\beta}\bar{x}_n$.

The $\hat{\alpha}$ and $\hat{\beta}$ notation is common as both *estimate* (using the $y_i$s), and *estimator* (using the $Y_i$s). For the estimators, you can check that $\hat{\beta}$ is unbiased, and that $\hat{\alpha}$ is too (using linearity of expectation). Variance needs a tweak because the $Y_i$s have different expectation. You can rely on $U_i=Y_i-\alpha-\beta x_i$ instead. You need to adjust by a weird $1/(n-2)$ factor to get an unbiased estimator for variance.

**Residuals** are the quantities $r_i=y_i-\hat{\alpha}-\hat{\beta} x_i$. They always sum up to 0 (not clear if this by design or a by-product of the method). Their scatterplot should be purely random. If instead there is some pattern, it suggests the simple linear regression model isn't appropriate (eg try multiple linear regression if it displays a somewhat parabolic shape). If they "fan out" with bigger $x_i$, that's called **heteroscedasticity**. There are techniques to get **homoscedasticity** (all $U_i$s have equal variance).

**Relation with maximum likelihood**: If the $U_i$s have a known distribution, the maximum likelihood principle can be applied. For $U_i$s following a normal distribution, it turns out it gives the same $\hat{\alpha}$ and $\hat{\beta}$ estimators to the least square method.
...
1. Therefore, if $T$ is an unbiased estimator for $\theta$ (ie that has a sampling distribution such that $\E{T}=\theta$), we expect over many experiments that (more than) 75% of the $(t-2\sigma_T,t+2\sigma_T)$ intervals will contain the true parameter $\theta$. Any of those intervals is called a 75% confidence interval for $\theta$.
1. Example: $T$ is the average of randvars, $t$ the sample mean ($\sigma_T=\sigma/\sqrt{n}$)

Note the assumption above that $\sigma_T$ is known.

## Shape of distribution known, variance known

Following on the example: take $\bar{X}_n=\frac{1}{n}\sum X_i$ where $X_i \sim N(\mu,\sigma^2)$ and iid. You know from before that $X_1+...+X_n\sim N(n\mu,n\sigma^2)$, and, scaling by $\frac{1}{n}$, that $\bar{X}_n\sim N\left(\frac{1}{n}n\mu,\left(\frac{1}{n}\right)^2 n \sigma^2\right)$. In other words, $\bar{X}_n\sim N(\mu,\frac{\sigma^2}{n})$. (With the central limit theorem giving this as well for non-normal $X_i$s and large $n$.)

$\E{\bar{X}_n}=\mu$ so $\bar{X}_n$ is an unbiased estimator for $\mu$, and $(\bar{x}_n-2\frac{\sigma}{\sqrt{n}},\bar{x}_n+2\frac{\sigma}{\sqrt{n}})$ is a 95% confidence interval for the true $\mu$. (Since $\bar{X}_n\sim N(\mu,\frac{\sigma^2}{n})$, $P(\mu\in(\bar{X}_n-2\frac{\sigma}{\sqrt{n}},\bar{X}_n+2\frac{\sigma}{\sqrt{n}}))\approx 95\%$.)


## Shape of distribution known, variance unknown

If you don't know $\sigma^2$ (the population variance), you don't know $\sigma_T^2$ (the sampling distribution variance), so you can't compute a confidence interval the same way. One remaining possibility is to use the sample variance $S_n^2=\frac{1}{n-1}\sum(X_i-\bar{X}_n)^2$ instead. However, the probabilities change, something something student t.

## Shape of distribution unknown
