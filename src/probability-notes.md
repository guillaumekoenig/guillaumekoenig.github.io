...
:   $P(Z≤z)=Φ(z)$. $Φ$ is the CDF of the *standard* ($μ=0$, $σ=1$)
    ...
    computations with random variables). A datapoint $z$ value (its
...
The quantile function is the inverse function of the CDF. Note the CDF
doesn't always have an inverse function, but the quantile function can
still be defined in a meaningful way.

----------------------------------------------------------------------

# Simulation

Produce a generator whose output follows a random variable $X$ with
cumulative distribution function $F_X : ℝ → [0,1]$. You are given $U
\sim U(0,1)$.
